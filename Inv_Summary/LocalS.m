...
for i = 1:2*(m+n)  % Control the number of iteration.
  ...
  if unopen > 0
    ...
    [opt_sol1,cost1] = add0(fi,trans,ind_open,ind_unopen,m,n);
  end
  % Define the swap-move   p(m-p) kinds of local search
  if unopen > 0

    [opt_sol2,cost2] = swap(fi,trans,ind_open,ind_unopen,m,n);
  end

% Define the remove   m-p  kinds of local search
  if open1 > 1

    [opt_sol3,cost3] = remove0(fi,trans,ind_open,ind_unopen,m,n);
  end
